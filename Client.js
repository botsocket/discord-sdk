...
module.exports = class Client {
      ...
      this.event({
        ...
        handler: (_client, message) => {
          ...
          for (const match of matches) {
            const { validation, failAction, handler } = match.definition.data;

            if (Jade.isSchema(validation?.args)) {
              ...
              if (result.errors) {
                ...
                } else if (failAction === "ignore") {
                  return;
                } else if (typeof failAction === "function") {
                  return failAction(message, result, "args");
                }
              }
            }

            if (Jade.isSchema(validation?.flags)) {
              ...
              if (result.errors) {
                if (!failAction || failAction === "error") {
                  ...
                  return failAction(message, result, "flags");
                }
              }
            }
  ...
  event(...events) {
    for (const event of events) {
      this._core.client[event.once ? "once" : "on"](event.name, (...args) =>
        event.handler(this._core.client, ...args)
      );
    }
  }
